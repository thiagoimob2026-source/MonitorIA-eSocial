...
# ESocialReport import moved to local methods to prevent startup hangs
from tkinter import filedialog, messagebox
import re
from event_templates import (
    generate_s1200_xml, generate_s1202_xml, generate_s1207_xml, generate_s1210_xml,
    generate_s3000_xml, generate_s1000_xml, parse_esocial_xml
)
...
class RubricGrid(ctk.CTkFrame):
    ...
    def get_data(self):
        ...
        for r in self.rows:
            ...
            if c:
                data.append({'codRubr': c, 'vrRubr': v, 'ideTabRubr': t})
        return data
...
class QuickHistoryPanel(ctk.CTkFrame):
    def __init__(self, parent, app, evt_type=None, **kwargs):
        ...
        title = f"Histórico ({evt_type})" if evt_type else "Histórico Geral"
        ctk.CTkLabel(header, text=title, font=ctk.CTkFont(weight="bold")).pack(side="left")
        
        # Toolbar
        toolbar = ctk.CTkFrame(self, fg_color="transparent")
        toolbar.pack(fill="x", padx=5, pady=2)
        ctk.CTkButton(toolbar, text="🔄", width=30, command=self.refresh, fg_color="#27AE60").pack(side="left", padx=2)
        ctk.CTkButton(toolbar, text="🔎 Ver", width=60, command=self.view_details, fg_color="#F39C12").pack(side="left", padx=2)
        ctk.CTkButton(toolbar, text="📝 Carreg", width=70, command=self.load, fg_color="#2E86C1").pack(side="left", padx=2)
        ctk.CTkButton(toolbar, text="🚀 Enviar", width=70, command=self.send, fg_color="green").pack(side="left", padx=2)
        ctk.CTkButton(toolbar, text="🗑️ Excluir", width=70, command=self.delete, fg_color="#C0392B").pack(side="left", padx=2)
    ...
    def refresh(self):
        ...
        self.tree.tag_configure("success", foreground="#10B981") # Emerald
        self.tree.tag_configure("error", foreground="#EF4444")   # Red
        self.tree.tag_configure("pending", foreground="#F59E0B") # Amber
        self.tree.tag_configure("default", foreground="#F8FAFC") # White
        
        history = self.app.db.get_history(evt_type=self.evt_type)
        for h in history:
            status = str(h["status"])
            tag = "default"
            if "Aceito" in status or "Sucesso" in status:
                tag = "success"
            elif "Erro" in status or "Falha" in status or "Rejeitado" in status:
                tag = "error"
            elif "PENDENTE" in status:
                tag = "pending"
                
            self.tree.insert("", "end", values=(
                h["evt_id"], h["type"], h["cpf"], h["status"], 
                h["protocol"] or "-", h["nr_recibo"] or "-", h["timestamp"]
            ), tags=(tag,))

    def view_details(self):
        selected = self.tree.selection()
        if not selected: return
        # Temporarily swap tree so the app method reads from this panel's tree
        old_tree = self.app.tree
        self.app.tree = self.tree
        self.app.view_totalizer()
        self.app.tree = old_tree


    def load(self):
        selected = self.tree.selection()
        if not selected: return
        evt_id = self.tree.item(selected[0])["values"][0]
        old_tree = self.app.tree
        self.app.tree = self.tree
        self.app.load_for_edit()
        self.app.tree = old_tree
...
class ESocialApp(ctk.CTk):
    def __init__(self):
        ...
        self.appearance_menu.grid(row=7, column=0, padx=20, pady=(10, 20))
    ...
    def setup_s1200_tab(self):
        ...
        self.s1200_qhistory = QuickHistoryPanel(right_col, self, evt_type="S-1200")
        ...
        self.form_frame = ctk.CTkFrame(page)
        self.form_frame.pack(fill="x", padx=20, pady=10)

        # Usando grid para 2 colunas
        self.form_frame.columnconfigure((0, 1), weight=1)

        fields = [
            ("CPF do Trabalhador", "cpfTrab", "", "Obrigatório"),
            ("Período (AAAA-MM)", "perApur", "2026-02", "Ex: 2026-02"),
            ("Nome (Opcional)", "nmTrab", "", ""),
            ("Nasc. (DDMMYYYY)", "dtNascto", "", "Ex: 01011980"),
            ("Matrícula (Opcional)", "matricula", "", ""),
            ("Lotação", "codLotacao", "1", "1"),
            ("Código CBO", "codCBO", "214120", "214120"),
            ("Categoria", "codCateg", "701", "701"),
            ("Ind. Teto RGPS (S/N)", "indTetoRGPS", "N", "N")
        ]
        
        for i, (text, key, placeholder, default_val) in enumerate(fields):
            row = i // 2
            col = i % 2
            ...
            ctk.CTkLabel(f, text=text, width=120, anchor="w").pack(side="left")
            e = ctk.CTkEntry(f, placeholder_text=placeholder)
            if default_val and default_val != "Obrigatório":
                e.insert(0, default_val)
            e.pack(side="right", fill="x", expand=True)
            self.s1200_entries[key] = e
        ...
        self.gen_csv_btn = ctk.CTkButton(batch_frame, text="Gerar Planilha Modelo (CSV)", command=self.generate_csv_template, height=35)
    ...
    def setup_s1202_tab(self):
        ...
        self.s1202_qhistory = QuickHistoryPanel(right_col, self, evt_type="S-1202")
        ...
        help_s1202 = ctk.CTkLabel(page, text="💡 Ativos RPPS: Matrícula é obrigatória. Nome/Nasc são opcionais se houver matrícula.", font=ctk.CTkFont(size=11), text_color="gray")
        help_s1202.pack(padx=20, pady=5, anchor="w")

        self.s1202_entries = {}
        
        # Identity Grid
        grid_frame = ctk.CTkFrame(page)
        grid_frame.pack(fill="x", padx=20, pady=10)
        grid_frame.columnconfigure((0, 1), weight=1)

        fields = [
            ("CPF do Servidor", "cpfTrab", "", "Obrigatório"),
            ("Competência (AAAA-MM)", "perApur", "2026-02", "2026-02"),
            ("Nome do Servidor", "nmTrab", "", ""),
            ("Nasc. (DDMMYYYY)", "dtNascto", "", ""),
            ("Matrícula / ID", "matricula", "", ""),
            ("Categoria (301, 303...)", "codCateg", "301", "301"),
            ("Código CBO", "codCBO", "214120", "214120"),
            ("Lotação", "codLotacao", "1", "1"),
            ("Plano RPPS (1-Prev, 2-Fin)", "tpPlanRPPS", "1", "1"),
            ("Ind. Teto RGPS (S/N)", "indTetoRGPS", "N", "N")
        ]
        
        for i, (text, key, placeholder, default) in enumerate(fields):
            row = i // 2
            col = i % 2
            ...
            e.pack(side="right", fill="x", expand=True)
            self.s1202_entries[key] = e
        
        ctk.CTkLabel(page, text="Gerenciador de Demonstrativos (ideDmDev)", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=(15, 5))
        self.s1202_dm_manager = DmDevManager(page)
        self.s1202_dm_manager.pack(fill="x", padx=20, pady=5)
        
        ctk.CTkButton(page, text="Salvar S-1202 (Ativo) na Fila", command=self.process_s1202, height=45, fg_color="#27AE60").pack(pady=20)
    ...
    def setup_s1207_tab(self):
        ...
        self.s1207_qhistory = QuickHistoryPanel(right_col, self, evt_type="S-1207")
        self.s1207_qhistory.pack(fill="both", expand=True)
        
        # Header with Import
        h_frame = ctk.CTkFrame(page, fg_color="transparent")
        h_frame.pack(fill="x", padx=20, pady=5)
        ctk.CTkLabel(h_frame, text="S-1207 - Remuneração de Beneficiários (RPPS)", font=ctk.CTkFont(size=20, weight="bold")).pack(side="left")
        ctk.CTkButton(h_frame, text="Importar XML para Retificar", command=lambda: self.import_rect_xml("S-1207"), fg_color="#F39C12").pack(side="right", padx=5)
        ctk.CTkButton(h_frame, text="Excluir via XML (S-3000)", command=lambda: self.import_exclude_xml("S-1207"), fg_color="#C0392B").pack(side="right")
        ...
        help_s1207 = ctk.CTkLabel(page, text="💡 Benefícios: Use o Número do Benefício no campo Matrícula.", font=ctk.CTkFont(size=11), text_color="gray")
        help_s1207.pack(padx=20, pady=5, anchor="w")
        ...
        fields = [
            ("CPF do Beneficiário", "cpfTrab", "", "Obrigatório"),
            ("Competência (AAAA-MM)", "perApur", "2026-02", "2026-02"),
            ("Nome Completo", "nmTrab", "", ""),
            ("Nasc. (DDMMYYYY)", "dtNascto", "", ""),
            ("Número do Benefício", "matricula", "", ""),
            ("Categoria (302, 309...)", "codCateg", "309", "309"),
            ("Lotação / Local", "codLotacao", "1", "1")
        ]
        ...
        for i, (text, key, placeholder, default) in enumerate(fields):
            ...
            if default: e.insert(0, default)
            e.pack(side="right", fill="x", expand=True)
            self.s1207_entries[key] = e
            
        ctk.CTkLabel(page, text="Gerenciador de Demonstrativos (ideDmDev)", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=(15, 5))
        self.s1207_dm_manager = DmDevManager(page)
        self.s1207_dm_manager.pack(fill="x", padx=20, pady=5)

        ctk.CTkButton(page, text="Salvar S-1207 (Benefício) na Fila", command=self.process_s1207, height=45, fg_color="#8E44AD").pack(pady=20)
    ...
    def setup_s1210_tab(self):
        ...
        self.s1210_qhistory = QuickHistoryPanel(right_col, self, evt_type="S-1210")
        ...
        fields = [
            ("CPF do Beneficiário", "cpfTrab", "", "Obrigatório"),
            ("Período Apuração", "perApur", "2026-02", "2026-02")
        ]
    ...
    def setup_s3000_tab(self):
        ...
        # Form
        self.s3000_frame = ctk.CTkFrame(page)
        self.s3000_frame.pack(fill="x", padx=20, pady=10)

        fields = [
            ("Evento a excluir (Ex: S-1200)", "tpEvento", "S-1200", "S-1200"),
            ("Número do Recibo (nrRecEvt)", "nrRecEvt", "1.2.000000...0", ""),
            ("CPF do Trabalhador", "cpfTrab", "000.000.000-00", ""),
            ("Competência (Ex: 2026-02)", "perApur", "2026-02", "")
        ]

        self.s3000_entries = {}
        for text, key, placeholder, default_val in fields:
            f = ctk.CTkFrame(self.s3000_frame, fg_color="transparent")
            f.pack(fill="x", padx=10, pady=5)
            l = ctk.CTkLabel(f, text=text, width=220, anchor="w")
            l.pack(side="left")
            e = ctk.CTkEntry(f, placeholder_text=placeholder)
            if default_val:
                e.insert(0, default_val)
            e.pack(side="right", fill="x", expand=True)
            self.s3000_entries[key] = e
    ...
    def process_s1200(self):
        ...
        data.update({
            ...
            'nrInscEstab': data.get('nrInscEstab') or self.config_data.get("nrInsc", ""),
        ...
        xml = generate_s1200_xml(data)
        from lxml import etree
        evt_id = etree.fromstring(xml.encode('utf-8'))[0].get("Id")
        self.db.save_event(None, evt_id, "S-1200", data['cpfTrab'], xml)
        self.save_xml_auto(xml, "S-1200", data['cpfTrab'])
        self.log(f"SUCESSO: S-1200 ({'RETIF' if data['indRetif']=='2' else 'ORIG'}) salvo como PENDENTE."); self.refresh_history()

    def generate_csv_template(self):
        filename = filedialog.asksaveasfilename(defaultextension=".csv", initialfile="modelo_s1200.csv", filetypes=[("CSV files", "*.csv")])
        if not filename:
            return
            
        header = ["CPF", "Nome", "Nascimento_DDMMYYYY", "Periodo", "Categoria", "CBO", "Matricula", "CodLotacao", "IDE_DM_DEV", "CodRubr", "TabelaRubrica", "VR_REM", "tpPlanRPPS", "indTetoRGPS"]
        try:
            with open(filename, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f, delimiter=';')
                writer.writerow(header)
                # Exemplo
                writer.writerow(["12345678909", "Nome Exemplo", "01011980", "2026-02", "701", "214120", "", "1", "001", "1000", "contindi", "1500.00", "1", "N"])
            self.log(f"Planilha modelo gerada com sucesso em: {filename}")
        except Exception as e:
            self.log(f"Erro ao gerar planilha modelo: {e}")
    ...
    def populate_form_from_data(self, target_tab, data):
        ...
        if target_tab == "S-1200":
            self.s1200_ind_retif.set(f"{data.get('indRetif', '1')} - {'Retificadora' if data.get('indRetif')=='2' else 'Original'}")
            self.s1200_nr_rec.delete(0, "end"); self.s1200_nr_rec.insert(0, data.get('nrRecEvt', ''))
            for k, v in data.items():
                if k in self.s1200_entries:
                    self.s1200_entries[k].delete(0, "end"); self.s1200_entries[k].insert(0, str(v) if v is not None else "")
            self.s1200_dm_manager.clear()
            for dm in data.get('demonstrativos', []):
                item = self.s1200_dm_manager.add_dm(dm['ideDmDev'])
                item.rubric_grid.clear()
                for r in dm.get('rubrics', []):
                    item.rubric_grid.add_row(r['codRubr'], r['vrRubr'], r.get('ideTabRubr', 'contindi'))
                    
        elif target_tab == "S-1202":
            self.s1202_ind_retif.set(f"{data.get('indRetif', '1')} - {'Retificadora' if data.get('indRetif')=='2' else 'Original'}")
            self.s1202_nr_rec.delete(0, "end"); self.s1202_nr_rec.insert(0, data.get('nrRecEvt', ''))
            for k, v in data.items():
                if k in self.s1202_entries:
                    self.s1202_entries[k].delete(0, "end"); self.s1202_entries[k].insert(0, str(v) if v is not None else "")
            self.s1202_dm_manager.clear()
            for dm in data.get('demonstrativos', []):
                item = self.s1202_dm_manager.add_dm(dm['ideDmDev'])
                item.rubric_grid.clear()
                for r in dm.get('rubrics', []):
                    item.rubric_grid.add_row(r['codRubr'], r['vrRubr'], r.get('ideTabRubr', 'contindi'))

        elif target_tab == "S-1207":
            self.s1207_ind_retif.set(f"{data.get('indRetif', '1')} - {'Retificadora' if data.get('indRetif')=='2' else 'Original'}")
            self.s1207_nr_rec.delete(0, "end"); self.s1207_nr_rec.insert(0, data.get('nrRecEvt', ''))
            for k, v in data.items():
                if k in self.s1207_entries:
                    self.s1207_entries[k].delete(0, "end"); self.s1207_entries[k].insert(0, str(v) if v is not None else "")
            self.s1207_dm_manager.clear()
            for dm in data.get('demonstrativos', []):
                item = self.s1207_dm_manager.add_dm(dm['ideDmDev'])
                item.rubric_grid.clear()
                for r in dm.get('rubrics', []):
                    item.rubric_grid.add_row(r['codRubr'], r['vrRubr'], r.get('ideTabRubr', 'contindi'))

        elif target_tab == "S-1210":
            self.s1210_ind_retif.set(f"{data.get('indRetif', '1')} - {'Retificadora' if data.get('indRetif')=='2' else 'Original'}")
            self.s1210_nr_rec.delete(0, "end"); self.s1210_nr_rec.insert(0, data.get('nrRecEvt', ''))
            for k, v in data.items():
                if k in self.s1210_entries:
                    self.s1210_entries[k].delete(0, "end"); self.s1210_entries[k].insert(0, str(v) if v is not None else "")
            self.pgto_manager.clear()
            for pgto in data.get('pagamentos', []):
                item = self.pgto_manager.add_pgto()
                for key_ui, val in pgto.items():
                    if hasattr(item, key_ui):
                        attr = getattr(item, key_ui)
                        if isinstance(attr, ctk.CTkEntry):
                            attr.delete(0, "end"); attr.insert(0, str(val) if val is not None else "")
                if 'demonstrativos' in pgto:
    ...
    def process_s1202(self):
        ...
        xml = generate_s1202_xml(data)
        from lxml import etree
        evt_id = etree.fromstring(xml.encode('utf-8'))[0].get("Id")
        self.db.save_event(None, evt_id, "S-1202", data['cpfTrab'], xml)
        self.save_xml_auto(xml, "S-1202", data['cpfTrab'])
        self.log(f"SUCESSO: S-1202 (Ativo) salvo como PENDENTE."); self.refresh_history()
    ...
    def process_s1207(self):
        ...
        xml = generate_s1207_xml(data)
        from lxml import etree
        evt_id = etree.fromstring(xml.encode('utf-8'))[0].get("Id")
        self.db.save_event(None, evt_id, "S-1207", data['cpfTrab'], xml)
        self.save_xml_auto(xml, "S-1207", data['cpfTrab'])
        self.log(f"SUCESSO: S-1207 (Benefício) salvo como PENDENTE."); self.refresh_history()

    def process_s1210(self):
        data = {k: e.get().strip() for k, e in self.s1210_entries.items()}
        data['pagamentos'] = self.pgto_manager.get_data()
        data.update({
            'tpAmb': self.config_data.get("tpAmb", "1"),
            'nrInsc': self.config_data.get("nrInsc", ""),
            'tpInsc': self.config_data.get("tpInsc", "1"),
        ...
        xml = generate_s1210_xml(data)
        from lxml import etree
        evt_id = etree.fromstring(xml.encode('utf-8'))[0].get("Id")
        self.db.save_event(None, evt_id, "S-1210", data['cpfTrab'], xml)
        self.save_xml_auto(xml, "S-1210", data['cpfTrab'])
        self.log(f"SUCESSO: S-1210 (Pagamento) salvo como PENDENTE."); self.refresh_history()

    def process_s1200_batch(self):
        filenames = filedialog.askopenfilenames(title="Selecione as planilhas CSV", filetypes=[("CSV files", "*.csv")])
        if not filenames:
            return
            
        total_count = 0
        self.log(f"--- Iniciando processamento massivo ({len(filenames)} arquivos) ---")
        ...
        for filename in filenames:
            try:
                with open(filename, 'r', encoding='utf-8') as f:
                    ...
                    file_count = 0
                    for row in reader:
                        cpf = row.get("CPF", "").strip().replace(".", "").replace("-", "")
                        if not cpf: continue
                        
                        rubrics_list = []
                        vr_rem = row.get("VR_REM", "0.00").replace(",", ".")
                        vr_inss = row.get("VR_INSS", "0.00").replace(",", ".")
                        vr_iss = row.get("VR_ISS", "0.00").replace(",", ".")
                        vr_irpf = row.get("VR_IRPF", "0.00").replace(",", ".")
                        
                        # Nova lógica flexível: Se CodRubr estiver presente, usa ele.
                        custom_cod = row.get("CodRubr", "").strip()
                        custom_tab = row.get("TabelaRubrica", "contindi").strip() or "contindi"
                        
                        if custom_cod and float(vr_rem) > 0:
                            rubrics_list.append({'codRubr': custom_cod, 'vrRubr': vr_rem, 'ideTabRubr': custom_tab})
                        else:
                            # Fallback legado para códigos fixos
                            if float(vr_rem) > 0:
                                rubrics_list.append({'codRubr': 'REMPRESTSERV01', 'vrRubr': vr_rem, 'ideTabRubr': 'ContIndi'})
                            if float(vr_inss) > 0:
                                rubrics_list.append({'codRubr': 'DEDINSSPRESTSERV02', 'vrRubr': vr_inss, 'ideTabRubr': 'ContIndi'})
                            if float(vr_iss) > 0:
                                rubrics_list.append({'codRubr': 'DEDISSPRESTSERV01', 'vrRubr': vr_iss, 'ideTabRubr': 'ContIndi'})
                            if float(vr_irpf) > 0:
                                rubrics_list.append({'codRubr': 'RETIRPFPRESTSERV01', 'vrRubr': vr_irpf, 'ideTabRubr': 'ContIndi'})
                        ...
                        data = {
                            ...
                            'cpfTrab': cpf,
                            'nmTrab': row.get("Nome", ""),
                            'dtNascto': row.get("Nascimento_DDMMYYYY", ""),
                            'perApur': row.get("Periodo", ""),
                            'nrInscEstab': self.config_data.get("nrInsc", ""),
                            'codLotacao': row.get("CodLotacao", ""),
                            'matricula': row.get("Matricula", ""),
                            'codCBO': row.get("CBO", ""),
                            'ideDmDev': row.get("ID", ""),
                            'codCateg': row.get("Categoria", ""),
                            'tpPlanRPPS': row.get("tpPlanRPPS", "1"),
                            'indTetoRGPS': row.get("indTetoRGPS", "N"),
                            'rubrics': rubrics_list
                        }
                        
                        categ = data['codCateg']
                        if categ in ['301', '302', '303', '307', '308', '309']:
                            xml_content = generate_s1202_xml(data)
                            evt_type = "S-1202"
                        else:
                            xml_content = generate_s1200_xml(data)
                            evt_type = "S-1200"

                        from lxml import etree
                        xml_tree = etree.fromstring(xml_content.encode('utf-8'))
                        evt_id = xml_tree[0].get("Id") if len(xml_tree) > 0 else f"CSV_{total_count}_{cpf}"
                        
                        self.db.save_event(None, evt_id, evt_type, cpf, xml_content)
                        self.save_xml_auto(xml_content, evt_type, cpf)
                        file_count += 1
                        total_count += 1
                    
                    self.log(f"Arquivo {os.path.basename(filename)} finalizado: {file_count} eventos salvos.")
            except Exception as e:
                self.log(f"Erro ao processar arquivo {filename}: {e}")

        self.log(f"--- Fim do processamento Massivo. {total_count} eventos salvos no Histórico. ---")
        self.tabview.set("Histórico")
        self.refresh_history()
    ...
    def load_xml_for_correction(self):
        ...
        try:
            ...
            # Detect Type
            evt_type = "S-1200"
            if "evtServRPPS" in content: evt_type = "S-1202"
            elif "evtBeneficio" in content: evt_type = "S-1207"
    ...
    def send_correction_final(self):
        ...
        try:
            if evt_type == "S-1200": xml = generate_s1200_xml(data)
            elif evt_type == "S-1202": xml = generate_s1202_xml(data)
            elif evt_type == "S-1207": xml = generate_s1207_xml(data)
            elif evt_type == "S-1210": xml = generate_s1210_xml(data)
            else: raise Exception("Tipo de evento não suportado para reenvio automático.")
    ...
    def setup_history_tab(self):
        ...
        self.main_history = QuickHistoryPanel(tab, self, fg_color="transparent")
